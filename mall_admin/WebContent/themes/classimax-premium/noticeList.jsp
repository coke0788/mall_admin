...
<section class="blog section">
	<div class="container">
		<div class="row">
			...
			<a href="<%=request.getContextPath()%>/themes/classimax-premium/insertNoticeForm.jsp?managerId=<%=manager.getManagerId()%>"><button type="button" class="btn btn-outline-primary py-1 float-right">EDIT</button></a>
			<!-- 페이지갯수 설정 -->
			<form action="<%=request.getContextPath()%>/themes/classimax-premium/noticeList.jsp" method="post">
				...
				<article>
					...
					<!-- Post Image -->
						...
						<h3>No. <%=n.getNoticeNo()%>&nbsp; &nbsp;│<%=n.getNoticeTitle()%></h3>
						<ul class="list-inline">
							<li class="list-inline-item">by <%=manager.getManagerId()%></li>
							<li class="list-inline-item"><%=n.getNoticeDate().substring(0,11)%></li>
						</ul>
						<p class=""><%=n.getNoticeContent()%></p>
						<!-- Read more button -->
						<a href="<%=request.getContextPath()%>/themes/classimax-premium/noticeOne.jsp?noticeNo=<%=n.getNoticeNo()%>" class="btn btn-transparent py-1 float-right">Read More</a>
					<%
						}
					%>
				</article>
